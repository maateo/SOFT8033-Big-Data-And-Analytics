...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    ...
    for input in my_input_stream:
        ...
        day_hour = key_value[0]
        ran_out_amount = key_value[1]

        if day_hour in outputs_dictionary:
            outputs_dictionary[day_hour] = outputs_dictionary[day_hour] + ran_out_amount
            total_ran_outs = total_ran_outs + ran_out_amount
        else:
            outputs_dictionary[day_hour] = ran_out_amount

    for output in sorted(outputs_dictionary.keys(), key=lambda item: outputs_dictionary[item], reverse=True):
        percentage_of_run_outs = float(outputs_dictionary[output] / my_reducer_input_parameters[0]) * 100
        string_to_write = output + "\t(" + str(outputs_dictionary[output]) + ", " + str(percentage_of_run_outs) + ")\n"
